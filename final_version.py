...
class ACCNode(Node):
    def __init__(self):
        ...
        self.create_subscription(ImuPose, '/InertialData', self.imu_callback, 10) # Vehicle_Data
        self.create_subscription(RadarTrackList, '/radar_processed', self.radar_callback, 10) # radar_tracks
    ...
    def imu_callback(self, msg):  ##### change this function
    ...
    def control_loop(self):
        ...
        if self.lead_distance is None or self.lead_speed is None:
    	    # Smooth decay using sigmoid around 7.5 m/s
            k = 2.0  # smoothness factor (higher = sharper transition)
            target_acc = cruise_acc / (1 + math.exp(k * (self.ego_velocity - 7.5)))
            self.filtered_acc = self.alpha * target_acc + (1 - self.alpha) * self.filtered_acc
            self.publish_control(self.filtered_acc)
            return
        ...
        self.filtered_acc = self.alpha * desired_acc + (1 - self.alpha) * self.filtered_acc
